...
public class LiveController {
    ...
    @GetMapping("/lives/cat/{categoria}")
    public ResponseEntity<List<LiveDocument>> getCategoria(@PathVariable(value="categoria") String categoria){
        List<LiveDocument> lives=liveService.findByCategoria(categoria);
        if(lives.isEmpty()){
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else{
            return new ResponseEntity<List<LiveDocument>>(lives, HttpStatus.OK);
        }
    }

    @GetMapping("/lives/videos")
    public ResponseEntity<List<LiveDocument>> findAll(){
        List<LiveDocument> lives=liveService.findAll();
        if(lives.isEmpty()){
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else{
            return new ResponseEntity<List<LiveDocument>>(lives, HttpStatus.OK);
    ...
    public ResponseEntity<LiveDocument> updateLive(@PathVariable(value="id") String id,
        ...
        if(!liveO.isPresent()) {
            ...
            liveDocument.setRegistrationDate(LocalDateTime.now());
